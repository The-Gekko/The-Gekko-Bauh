...
| `eopkg/` | Sí (si hay `eopkg`) | Propia del fork. `controller.py` adapta Solus/eopkg (búsqueda, instalados con versión, `list-upgrades`, instalar/eliminar/actualizar con `-y`; `-N` solo desactiva el color) y sincroniza los repositorios en `prepare()`; `commands.py` construye las líneas de comando con los alias nativos (`it`, `rmf`, `up`, `ur`, `li`, ...) como listas de argumentos; `parsers.py` analiza la salida de texto de eopkg (listas, `info`, progreso de instalación) y rejunta la línea `Name : ..., version: ..., release: ...` cuando eopkg la parte; `index.py` calcula cuándo se sincronizó por última vez el índice de `/var/lib/eopkg/index` y si toca repetirlo (una vez al día), con la marca propia en `~/.cache/gekko-bauh/eopkg/repo_sync`; `worker.py` (`SyncRepositories`) ejecuta ese `sudo eopkg ur` en segundo plano como tarea de arranque; `model.py` representa paquetes; `config.py` (`search_limit`, `command_timeout`, `sync_repos_before_upgrade`, `sync_repos_startup`). |
...
| `tests/gems/eopkg/` | Construcción de comandos, transacciones y ajustes del controlador (`test_commands.py`); lectura de instalados (`test_controller.py`); análisis de la salida de eopkg, incluida la línea `Name` partida (`test_parsers.py`); estado de sincronización del índice, tarea de arranque y avisos (`test_index.py`). |
