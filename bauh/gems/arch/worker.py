...
GLOBAL_MAKEPKG_DIR = '/etc/makepkg.conf.d'
GENERATED_BY_HEADER = '# <generated by bauh>'
# variables cuya linea original se elimina del makepkg.conf copiado: la optimizacion solo debe
# aplicarse si no la define ni la distribucion (/etc/makepkg.conf.d) ni el usuario
GUARDED_OPTIMIZATION_VARS = ('MAKEFLAGS', 'COMPRESSXZ', 'COMPRESSZST')
# detecta una compresion ya configurada como multihilo: '--threads=N' (xz/zstd) o '-T N' / '-TN'
RE_COMPRESS_MULTITHREAD = re.compile(r'(--threads|-T\s*\d+)')
...
class ArchCompilationOptimizer(Thread):
    ...
    @staticmethod
    def gen_external_config_sourcing(conf_dir: str = GLOBAL_MAKEPKG_DIR) -> str:
        """
        Devuelve el fragmento de shell que carga los ficheros de configuracion que makepkg
        ignora cuando se le pasa '--config': /etc/makepkg.conf.d/*.conf y el makepkg.conf
        personal del usuario (libmakepkg solo carga estos ultimos cuando el fichero recibido
        es exactamente /etc/makepkg.conf). Se respeta el orden de 'source_makepkg_config'.

        'conf_dir' existe para poder probar el fragmento con un directorio de ejemplo.
        """
        return f'''# ficheros que makepkg ignora al recibir '--config': se cargan aqui para no perder
# ni la configuracion de la distribucion ni la del usuario
if [ -d {conf_dir} ]; then
    for _bauh_conf in {conf_dir}/*.conf; do
        if [ -r "$_bauh_conf" ]; then
            source "$_bauh_conf"
        fi
    done
    unset _bauh_conf
...
if [ -n "$HOME" ]; then
    _bauh_user_conf="${{XDG_CONFIG_HOME:-$HOME/.config}}/pacman/makepkg.conf"
    if [ -r "$_bauh_user_conf" ]; then
        source "$_bauh_user_conf"
    elif [ -r "$HOME/.makepkg.conf" ]; then
        source "$HOME/.makepkg.conf"
    fi
    unset _bauh_user_conf
fi'''

    @staticmethod
    def guard_optimization(declaration: str) -> str:
        """
        Envuelve una optimizacion para que solo se aplique si la variable sigue sin valor
        despues de cargar la configuracion de la distribucion y la del usuario.
        """
        var = declaration.split('=')[0].strip()

        if var in GUARDED_OPTIMIZATION_VARS:
            return f'if [ -z "${{{var}[*]}}" ]; then\n    {declaration}\nfi'

        return declaration

    @classmethod
    def gen_custom_makepkg_content(cls, stripped_makepkg: str, optimizations: List[str],
                                   conf_dir: str = GLOBAL_MAKEPKG_DIR) -> str:
        """Compone el makepkg.conf optimizado que se pasara a makepkg con '--config'."""
        guarded = '\n'.join(cls.guard_optimization(o) for o in optimizations)
        sourcing = cls.gen_external_config_sourcing(conf_dir)
        return f'{stripped_makepkg}\n{sourcing}\n\n{GENERATED_BY_HEADER}\n{guarded}\n'
    ...
    def optimize(self):
        ...
        if os.path.exists(GLOBAL_MAKEPKG):
            ...
            if compress_xz:
                not_eligible = [f for f in compress_xz
                                if not f.startswith('#') and RE_COMPRESS_MULTITHREAD.search(f)]
            ...
            if compress_zst:
                not_eligible = [f for f in compress_zst
                                if not f.startswith('#') and RE_COMPRESS_MULTITHREAD.search(f)]
            ...
            if custom_makepkg and optimizations:
                custom_makepkg = self.gen_custom_makepkg_content(custom_makepkg, optimizations)
