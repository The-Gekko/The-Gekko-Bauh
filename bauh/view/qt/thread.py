...
class AsyncAction(QThread, ProcessWatcher):
    """Hilo base de las acciones asíncronas de la ventana principal.

    ``run()`` es una plantilla: ejecuta ``_run()`` (que implementan las subclases) y, si
    este lanza una excepción no controlada, la registra en el logger disponible, avisa al
    usuario mediante ``signal_message`` y garantiza que ``signal_finished`` se emite con el
    valor seguro de ``_get_error_result()``, de modo que la ventana nunca queda bloqueada
    ni PyQt aborta el proceso por una excepción sin capturar dentro del hilo.
    """
    ...
    def _run(self):
        """Cuerpo real de la acción. Las subclases sobrescriben este método en lugar de ``run()``."""
        raise NotImplementedError(f"{self.__class__.__name__} must implement '_run'")

    def _get_error_result(self) -> object:
        """Valor notificado por ``signal_finished`` cuando ``_run()`` falla de forma inesperada.

        Se evalúa antes de ejecutar ``_run()`` para que refleje el estado inicial de la acción
        (paquete, acción personalizada...) aunque ``_run()`` lo haya limpiado en un ``finally``.
        Las subclases lo redefinen para respetar el contrato que espera su manejador en la
        ventana; debe ser trivial y no lanzar excepciones.
        """
        return None

    def _get_logger(self) -> Logger:
        """Devuelve el logger de la aplicación si el hilo tiene acceso a él (atributo ``logger``
        o el contexto de su ``SoftwareManager``); en caso contrario, el logger de este módulo."""
        logger = getattr(self, 'logger', None)

        if isinstance(logger, Logger):
            return logger

        for attr in ('manager', 'man'):
            context = getattr(getattr(self, attr, None), 'context', None)
    ...
    def run(self):
        """Plantilla común a todos los hilos: nunca deja escapar una excepción de ``_run()``."""
        self._finished_notified = False
        error_result = None
        ...
        try:
            ...
            self._run()
        except Exception as e:  # noqa: BLE001 - frontera de errores: nada debe escapar del hilo
            self._handle_unexpected_error(e, error_result)

    def _handle_unexpected_error(self, error: Exception, error_result: object):
        """Registra la excepción, avisa al usuario y asegura la emisión de ``signal_finished``."""
        try:
            self._get_logger().exception("Unexpected error while running '%s'", self.__class__.__name__)
            self._notify_unexpected_error(error)
        except Exception:
            _LOGGER.exception("Could not report the unexpected error of '%s'", self.__class__.__name__)
        finally:
            if not self._finished_notified:
                self.notify_finished(error_result)

    def _notify_unexpected_error(self, error: Exception):
        """Muestra al usuario un aviso de error con los detalles técnicos de la excepción."""
        details = html.escape(f"{error.__class__.__name__}: {error}"[:500])
        self.show_message(title=self.i18n['error'].capitalize(),
                          body=f"<p>{self.i18n['action.failed']}</p><p>{details}</p>",
...
class UpgradeSelected(AsyncAction):
    ...
    def _write_summary_log(self, upgrade_id: str, requirements: UpgradeRequirements):
        try:
            ...
            self._get_logger().exception("Could not write the upgrade summary log '%s'", upgrade_id)
    ...
    def _get_error_result(self) -> object:
        return {'success': False, 'updated': 0, 'types': set(), 'id': None}
...
class RefreshApps(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'installed': [], 'total': 0, 'types': set()}
    ...
    def _run(self):
        try:
            ...
            self._get_logger().exception("Could not read the installed packages")
...
class UninstallPackage(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'success': False, 'removed': None, 'pkg': self.pkg}
    ...
    def _run(self):
        if self.pkg:
            ...
            try:
                ...
                self._get_logger().exception("Could not uninstall '%s'", self.pkg.model.name)
                self.notify_finished({'success': False, 'removed': None, 'pkg': self.pkg})
            finally:
                self.pkg = None
                self.root_pwd = None
...
class DowngradePackage(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'app': self.pkg, 'success': False}
...
class ShowPackageHistory(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'error': self.i18n['action.failed']}
...
class InstallPackage(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'success': False, 'installed': None, 'removed': None, 'pkg': self.pkg}
    ...
    def _run(self):
        if self.pkg:
            ...
            try:
                ...
                self._get_logger().exception("Could not launch '%s'", self.pkg.model.name)
            finally:
...
class CustomAction(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'success': False, 'pkg': self.pkg, 'action': self.custom_action,
                'error': None, 'error_type': MessageType.ERROR}
...
class ShowScreenshots(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        return {'pkg': self.pkg, 'screenshots': ()}
...
class IgnorePackageUpdates(AsyncAction):
    ...
    def _get_error_result(self) -> object:
        ignored = bool(self.pkg and self.pkg.model.is_update_ignored())
        return {'action': 'ignore_updates_reverse' if ignored else 'ignore_updates',
                'success': False, 'pkg': self.pkg}
    ...
    def run(self):
        if self.theme_key:
            ...
            try:
                ...
                _LOGGER.exception("Could not save the theme '%s' to the configuration file", self.theme_key)
